...
void CLKUpdate(uint8_t clock_freq)
{
	LPC_SYSCON->MAINCLKSEL = 0x00;  	// IRC oscillator
	...
	// set up PLL frequency and activate
	switch(clock_freq){
		case 6:
			LPC_SYSCON->SYSPLLCTRL = 0x25;	// multiplier of 6 gives 72 MHz
			break;
		case 5:
			LPC_SYSCON->SYSPLLCTRL = 0x24;	// multiplier of 5 gives 60 MHz
			break;
		case 4:
			LPC_SYSCON->SYSPLLCTRL = 0x23;	// multiplier of 4 gives 48 MHz
			break;
		case 3:
			LPC_SYSCON->SYSPLLCTRL = 0x42;	// multiplier of 3 gives 36 MHz
			break;
		case 2:
			LPC_SYSCON->SYSPLLCTRL = 0x41;	// multiplier of 2 gives 24 MHz
			break;
		case 1:
...
int main(void) {
    ...
    CLKInit(); // multiplier of 1 gives 12 MHz
    ...
    while (1) {
    	...
    	for (i = 0; i < cycles*2; i++) {
    		for (counter = 0; counter < 12000; counter++) asm("nop");
    		LPC_GPIO0->DATA ^= (0x01 << 7);
    	}
		//LPC_GPIO0->DATA |= (0x01 << 7);
    	...
    	for (i = 6; i >= 1; i--) {
    		CLKUpdate(i);
    		for (counter = 0; counter < 120000/i; counter++) asm("nop");

    	    //LPC_GPIO0->DATA &= ~(0x01 << 7); // signal begin
    		result = fibonacci(30);
    	    //LPC_GPIO0->DATA |= (0x01 << 7); // signal end
    	}
